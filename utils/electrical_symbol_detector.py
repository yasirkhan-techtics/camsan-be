...
def run_detection_pipeline(image_path: str,
    ...
    template = detector.select_template_roi(image_path, max_display_size=max_display_size)
    ...
    if save_output:
        ...
        # Save result image with detections using the new method
        ...
        template_filename = os.path.basename(image_path).rsplit('.', 1)[0] + '_template.png'
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python electrical_symbol_detector.py <image_path>")
        print("\nOptional arguments:")
